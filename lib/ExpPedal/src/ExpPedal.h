...
public:
    ...
    void begin(TwoWire* wireInstance, int sdaPin = -1, int sclPin = -1, uint8_t address = DEFAULT_ADC_ADDRESS);    // Initialize I2C and pins
    void update();   // Update both expression pedal and switches
    void attachCallback(void (*callback)(int)); // Attach callback for expression pedal changes
    int getValue(); // Get current smoothed value
